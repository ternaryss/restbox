...
/**
 * Unit tests for get movies handler.
 *
 * @author TSS
 */
public class GetMoviesTest {
  ...
  private final MovieDto[] mockedMoviesDto = {
      MovieDto.builder().movId(1).title("The Shawshank Redemption").genere("Drama")
          ...
          .country("USA").act(true).description("Test description").build(),
  ...
  public void setup() {
    ...
    handler = new GetMovies(movieRepo);
  }
  ...
  public void paginatedTest() {
    ...
    Mockito.when(movieRepo.findByMoviesFilter(filter)).thenReturn(new LinkedList<Movie>() {
      {
        add(mockedMovies[0]);
        add(mockedMovies[1]);
      }
    });
    Mockito.when(movieRepo.countByMoviesFilter(filter)).thenReturn(5l);
    GetMoviesCmd command = new GetMoviesCmd(filter);
    command = (GetMoviesCmd) handler.handle(command);
