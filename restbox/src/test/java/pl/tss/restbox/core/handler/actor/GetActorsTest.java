...
/**
 * Unit tests for get actors command handler.
 *
 * @author TSS
 */
public class GetActorsTest {
  ...
  public void setup() {
    ...
    handler = new GetActors(personRepo);
  }
  ...
  public void paginatedTest() {
    ...
    Mockito.when(personRepo.findByActorsFilter(filter)).thenReturn(new LinkedList<Person>() {
      {
        add(mockedActors[0]);
        add(mockedActors[1]);
      }
    });
    Mockito.when(personRepo.countByActorsFilter(filter)).thenReturn(5l);
    GetActorsCmd command = new GetActorsCmd(filter);
    command = (GetActorsCmd) handler.handle(command);
